...
Für das Ausrollen des MCP Servers in Kubernetes ist zunächst der Download und die Vorbereitung der Daten erforderlich. Dies wird per Kubernetes-Job mit dem Docker Image "deutsche-gesetze-mcp-crawl" realisiert. Dieses schreibt in ein persistentes Volume, welches anschliessend vom "deutsche-gesetze-mcp-server" Deployment eingebunden wird.

Die Ausführung des Crawl Jobs erfolgt mit:

`kubectl -n <namespace> apply -k deutsche-gesetze-mcp-crawl/production`
...
`kubectl -n <namespace> apply -k deutsche-gesetze-mcp-server/production`
